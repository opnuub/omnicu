...
fn main() -> eyre::Result<()> {
    ...
    const UNIHAN_GLOB: &[&str] = &["Unihan_IRGSources.txt"];
    ...
    let raw_content =
        std::fs::read_to_string(&irg_path).with_context(|| "Failed to read extracted IRG file")?;

    let filtered_content: String = raw_content
        .lines()
        .filter(|line| line.contains("kRSUnicode") || line.starts_with('#'))
        .collect::<Vec<&str>>()
        .join("\n");

    std::fs::write(&irg_path, filtered_content)
        .with_context(|| "Failed to write filtered IRG file")?;
...
use crate::{{AbstractFs, CldrCache, SerdeCache, SourceDataProvider, UnihanCache, TzdbCache}};
